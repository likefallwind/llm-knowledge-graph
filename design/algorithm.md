...
该句只有强调和共现，没有明确组成陈述。这说明关系裁判仍可能产生假阳性，尤其是 `part_of`。后续质量改进应优先收紧关系提示词、建立真实语料回归样本并按关系报告准确率，而不是把弱表达硬编码为大量特例。
...
这里的 `chunk_hash` 实际是处理指纹，包含正文 Chunk 哈希、Passage/抽取/裁判提示词版本、模型和每块实体/Claim 上限。算法或参数变化后，同一文本会重新处理，不会被旧 `done` 错误跳过。
...
但顶层必须能解析出一个 JSON 对象。格式错误直接记为本片段失败，不再调用
另一个 LLM 修补，以保留实验行为的可解释性。
