...
后续 20 Chunk 重跑只形成 3 条已物化 `prerequisite_of`。这不再被视为需要继续
调参的问题：学习前提比类属和组成更依赖作者的教学视角，低频是可接受结果。
该关系保留为稀疏附加信息，不设数量目标、不单独追求召回率，也不为它建设金标、
人工审核或统计评测。后续开发回到待定端点解析、Evidence 合并和语料覆盖主线。
...
`experiments/relation_two_stage_pilot.py` 在只读源库的固定样本上，分开验证两个问题：
同义表达的双向等价替换，以及 Assertion 语境中成立的单向粗化。前者在派生细图上
替换去重，保留原 Claim/Assertion/Evidence；后者生成独立映射，不注册同义 alias。
粗目录由归一后的细关系和代表事实生成，分配时隐藏旧关系名并展示完整目录。
映射按 Assertion 保存，原图的全部事实和证据在实验产物中保留。

该实验不改变抽取、在线 RelationType 归一或正式 schema 的行为。派生图使用实验专用
SQLite 表和 JSON 目录，不覆盖生产数据库。流程和限制见 `experiments/README.md`。

### 16.1 同义归一与粗类别标注（2026-09-05 目标修订）

`experiments/relation_synonyms_categories.py` 保留旧实验以便对照，采用不同语义：
字符 TF-IDF 与包含代表 Assertion 的 E5 候选并集，每路 top-12；LLM 逐关系找同义候选，
再分批覆盖候选两端全部 Assertion 核验。按使用次数和 ID 选择规范关系，替换必须有直接
源到规范关系的核验，不能沿候选链传递。一个 Claim 多条 Assertion 不一致时保留原细关系；
不会将局部同义自动注册为全局 alias。

归一后每60个细关系及代表事实生成最多15个局部候选，再结合全部细关系名统一成全局单层粗类别
（建议10–30，最多35），避免全部Assertion示例挤入一次长上下文。类别包含纳入、排除
和冲突优先规则。分类按 Assertion 语境保存，同时汇总细关系到粗类别的对应；多义用法可以
有不同类别。粗类别是检索组织标签，不是替代谓词，不执行三元组单向蕴含验证。使用单独提示词
检查分类边界，失败保留 pending；同模型复核覆盖率不代表独立准确率。

派生 SQLite 同时保存 source_rows、fine_relations、fine_claims、claim_lineage、
coarse_categories、assertion_classification，并提供 classified_facts 查询视图。源库只读，
不增加新事实，分别统计规范细关系和粗类别数，不能把二者相加解释为压缩后的谓词数。
所有 API 阶段共用一个 MiniMax-M3 客户端和总并发上限6；缓存、源码快照和输入哈希支持审计。

### 16.2 阶段收敛（2026-09-07）

后续关系组织采用规范细关系与独立粗类别两层表示，Assertion继续保存完整事实及证据。
细关系以合并同义表达、保留有意义区别为目标；粗类别支持汇总检索、浏览和编排，不作为
替代谓词，也不将细关系数与粗类别数相加解释为谓词压缩结果。研究不以零错误、100%覆盖
或固定细关系数量为门槛，以大部分正确及实际检索效果评价可用性。

本轮实验得到873种细关系和27个粗类别；具体覆盖和质量边界见
[收敛报告](../experiments/relation_synonyms_categories_report.md)。这是实验方案的收敛，
尚未修改正式schema或在线抽取流程。原始结果保留，审计中的10条撤回和4条待定建议尚未应用。
当前实现仍要求目标旧标签的全部用法通过同义复核；逐Claim的义项归一与全局别名判断
有待分开验证，不应将这一已知限制描述为已修复。后续优先用简短核心定义和代表性留出
样本评估误合并、漏合并及分类质量，避免为个别案例叠加提示词例外。
